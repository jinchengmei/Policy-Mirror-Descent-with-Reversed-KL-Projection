...
As it is shown in \cref{subsec:compeval}, the performance of SAC heavily relies on the re-parametrization trick, which again restricts SAC to the continuous setting.
...
A policy-based version of SAC will degenerates to MENT \citep{levine2018reinforcement}, which as shown in \cref{subsec:compeval} is significantly outperformed by REPMD.
...
Q-values.
Under the same assumption that a non-convex optimization
problem can be solved globally, SAC further obtains a global optimum property based on the idea of policy improvement algorithm.
Such property, while stronger than our fixed points guarantee at first sight, arguably is a natural corollary from the strong assumption.
One way to see it is consider its policy-based version.
As mentioned above, the policy-based version of SAC degenerates to MENT where this property becomes a tautology: assume that $\min_\pi \KL(\pi \| \pi^*_\tau)$ can be solved globally, then $\pi^*$ maximizes $\E_{\rho \sim \pi} r(\rho) + \tau \cH(\pi)$  globally.
