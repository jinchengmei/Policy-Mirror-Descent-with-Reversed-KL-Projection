...
\begin{proof}
	...
	{\bf (Global optimum inclusion)} The stationary point of $\oep_{\rho \sim \pi_\theta}{  r(\rho)}$ is the $\pi_\theta$ satisfies,
	\begin{equation}
	...
	\end{equation*}
	Let $c = 1 + \log{ \sum\limits_{\rho}{\pi_\theta(\rho) \exp\left\{r(\rho) / \tau\right\} } }$, \cref{eq:stationary_point_of_expected_reward} is equivalent with,
	\begin{equation}
	...
	\begin{split}
		...
		\iff \sum\limits_{\rho}{ \left[ \log{\pi_\theta(\rho)} - \log\left\{ \frac{ \pi_\theta(\rho) \exp\left\{r(\rho) / \tau\right\}}{ \sum\limits_{\rho}{\pi_\theta(\rho) \exp\left\{r(\rho) / \tau\right\} } } \right\} + 1 \right] \cdot \frac{d \pi_{\theta}(\rho)}{d \theta} } &= 0 \\
	...
	\end{split}
	\end{equation}
	The last equation in \cref{eq:stationary_point_of_expected_reward_cont} is the condition that the fixed points of PMD iteration \cref{eq:pmd} satisfy.
\end{proof}
