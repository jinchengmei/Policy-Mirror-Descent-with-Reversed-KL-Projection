...
\label{subsec:proof_of_repmdgradientestimate}
\begin{proof}
	Note that
	\begin{equation*}
...
\subsection{Proof of \cref{thm:rmacgradientestimate}}
\begin{proof}
...
\section{Implementation details of REPMD}
%\subsection{Learning }
...
We now provide some of the implementation details of REPMD.
...
\section{Implementation details of PMAC}
...
\end{thm}
\begin{proof}
See \cref{subsec:proof_of_repmdgradientestimate} for the proof of Theorem \ref{thm:repmdgradientestimate}.
\end{proof}
...
\section{Details of Learning Algorithm}

Details of the learning algorithms of REPMD and *** are provided in this section. 

\subsection{Details of REPMD}

Pseudocode for REPMD is presented in \cref{alg:repmd}. 

\begin{algorithm}[H]
	\caption{  The REPMD algorithm}
	\begin{algorithmic}[1]
...
Pseudocode for *** is presented in \cref{alg:rmac}. The major difference between *** and repmd in the lift step is that instead of sampling $K$ actions as described in \cref{alg:repmd}, **** only samples one action to construct $\bar{\pi}_{\tau,\tau^{\prime}}^*$, due to the fact both the soft-Q and state value function approximations are adopted. The value function approximations also make *** capable of using off-policy data from a reply buffer. 

\begin{algorithm}[H]
	\caption{\label{alg:rmac}  The **** algorithm}
	\begin{algorithmic}[1]
		...
		\FOR { $t=1,2,\ldots$ }
			...
			\STATE Sample a mini-batch of data $\gM=\{ (s_i, a_i, r_i, s'_i) \}_{i=1}^B$ from $\gD$
