...
As mentioned in the Introduction, our analyses focus on analyzing its properties in the \emph{non-convex} setting.
...
We present Policy Mirror Descent (PMD) in this section, a predecessor to REPMD.
Given a \emph{reference policy} $\refPi$ (usually the current policy), 
PMD learns $\pi_{\theta} $ by maximizing the relative entropy regularized expected reward:
%let 
\begin{equation}
...
\end{equation}
The relative entropy regularization has been widely used in the literature of online learning and constrained optimization problems \citep{nemirovskii1983problem,beck2003mirror}, mostly in the mirror descent algorithm.
Note that when $\refPi$ is a uniform distribution, \cref{eq:max_expected_reward_plus_relative_entropy} recovers the entropy regularized expected reward.
In this paper, we are interested in the non-convex setting, where $\pi$ is parametrized as a non-convex function of $\theta \in \real^d$ and thus $\Pi$ could be a non-convex set in the simplex.
%\begin{equation}
%\label{eq:max_expected_reward_plus_relative_entropy}
%\pi_{\theta_{t+1}} = \argmax\limits_{\pi_\theta \in \Pi}  \ENT(\pi_\theta; \tau, \pithetat). 
%\end{equation}
...
To better investigate its behavior, we first reformulate the problem as a lift-and-project procedure.
\begin{equation}
\label{eq:pmd}
\begin{split}
...
\end{split}
\end{equation}
\cref{prop:mirrordescent_projection} shows that even in the non-convex setting, the problems of  \cref{eq:max_expected_reward_plus_relative_entropy} and \cref{eq:pmd} are in fact equivalent.
\begin{prop}
...
%Note that although the lift step is non-convex in $\pi$, we can solve it analytically, where
%\begin{equation}
%\label{eq:pitaustar}
%\bar{\pi}_\tau^*(\rho) =  \frac{\refPi(\rho) \exp\left\{ r(\rho) / \tau \right\}}{ \sum_{\rho^{\prime}}{\refPi(\rho^{\prime}) \exp\left\{ r(\rho^{\prime}) / \tau \right\} } }.
%\end{equation}
The above reformulation leads to our PMD algorithm: Lift the current policy $\pi_{\theta_t}$ to $\bar{\pi}_\tau^*$, then perform multiple steps of gradient descent on the project step to update $\pi_{\theta_{t+1}}$. \footnote{To estimate this gradient, one would need to use the self-normalized importance sampling \cite{owen2013monte}. We omit its implementation details since PMD  is not our main algorithm, and similar techniques can be found in the implementation of REPMD in cref-tbd.}
...
The next proposition shows that despite of the non-convexity of $\Pi$, PMD still enjoys some desirable properties.
\begin{prop}
\label{prop:monoto_policymirrordescent}
Assume that $\pi_{\theta_{t}}$ is the update sequence. PMD satisfies the following properties for an arbitrary parametrization of $\pi$.
\begin{enumerate}
